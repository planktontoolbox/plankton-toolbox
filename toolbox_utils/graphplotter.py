...
class GraphPlotData(object):
    ...
    def add_plot(
        ...
        if (not y_array) or (len(y_array) == 0):
            toolbox_utils.Logging().warning(
                "GraphPlotData.add_plot() must contain at least an y-array. Plot not added."
            )
            raise UserWarning(
                "GraphPlotData.add_plot() must contain at least an y-array. Plot not added."
            )
    ...
    def get_min_max_values(self, array_name="x_array"):
        """ """
        result_min = None
        result_max = None
        for plotdict in self._plot_list:
            ...
            if not max:
                result_max = maxvalue
            if result_min > minvalue:
                result_min = minvalue
            if result_max < maxvalue:
                result_max = maxvalue
        #
...
class ChartBase(object):
    ...
    def __init__(self, plot_data=None, figure=None):
        ...
        if plot_data:
            if not isinstance(plot_data, GraphPlotData):
                toolbox_utils.Logging().error(
                    "Chart data object is not an instance of the class GraphPlotData. Plotting terminated."
                )
                raise UserWarning(
                    "Chart data object is not an instance of the class GraphPlotData. Plotting terminated."
                )
        ...
        if self._figure:
            self._pyplot_used = (
                False  # Embedded (in Qt, Tkinter, etc.) or used for file plotting.
            )
        else:
            self._pyplot_used = (
                True  # Use pyplot for tests and for 'manual' file creation.
            )
    ...
    def _init_plotting(self):
        ...
        if self._data.get_plot_data_info()["title"]:
            self._figure.suptitle(
                self._data.get_plot_data_info()["title"],
    ...
    def _format_array(self, data_type, data_array, data_format=""):
        ...
        if data_type == "integer":
            ...
            if len(failedconversions_set) > 0:
                toolbox_utils.Logging().warning(
                    'GraphPlotter.ChartBase: These values could not be converted to integer: "'
        ...
        #
            ...
            if len(failedconversions_set) > 0:
                toolbox_utils.Logging().warning(
                    'GraphPlotter.ChartBase: These values could not be converted to float: "'
                    + '", "'.join(sorted(failedconversions_set))
                    + '"'
                )
            # Masked arrays are needed for proper handling of missing values when plotting.
        ...
        #
            ...
            if len(failedconversions_set) > 0:
                toolbox_utils.Logging().warning(
                    'GraphPlotter.ChartBase: These values could not be converted to date or datetime: "'
                    + '", "'.join(sorted(failedconversions_set))
                    + '"'
                )
    ...
    def plot_chart(self, **kwargs):
        ...
        raise NotImplementedError(
            "The abstract method ChartBase.plot_chart() is not implement. "
        )
        ...
        **kwargs
        ...
        try:
            ...
            if not x_type in ["", "text", "integer", "float", "date", "datetime"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.LineChart: Plot skipped, X-axis type not valid: "
                    + x_type
                )
                raise UserWarning(
                    "GraphPlotter.LineChart: Plot skipped, X-axis type not valid: "
            ...
            if not y_type in ["", "integer", "float"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.LineChart: Plot skipped, Y-axis type not valid: "
                    + y_type
                )
                raise UserWarning(
                    "GraphPlotter.LineChart: Plot skipped, Y-axis type not valid: "
            ...
            if combined:
                ...
                for plotindex, plotdict in enumerate(plotlist):
                    ...
                    #
                        ...
                        x_masked_array = self._format_array(
                            "integer", x_array, x_format
                        )
                        y_masked_array = self._format_array(y_type, y_array, y_format)
                        #
                        if stacked:
                    ...
                    #
                        ...
                        if x_format:
                            subplot.xaxis.set_major_formatter(
                                mpl_dates.DateFormatter(x_format)
                            )
                        else:
                            subplot.xaxis.set_major_formatter(
                    ...
                    #
                        ...
                        toolbox_utils.Logging().warning(
                            "GraphPlotter.LineChart: Not a valid type for the x axis: "
                            + x_type
                        )
                        return
                # Legend.
                ...
                subplot.set_xlabel(
                    self._data.get_plot_data_info()["x_label"], size="medium"
                )
                subplot.set_ylabel(
                    self._data.get_plot_data_info()["y_label"], size="medium"
                )
            else:
                ...
                for plotindex, plotdict in enumerate(plotlist):
                    ...
                    y_array = plotdict["y_array"]
                    #
                    subplot = self._figure.add_subplot(
                        subplotcount, 1, plotindex + 1, sharex=sharesubplotxaxis
                    )
                    ...
                    #
                        ...
                        toolbox_utils.Logging().warning(
                            "GraphPlotter.LineChart: Not a valid type for the x axis. Type "
                            + x_type
                        )
                    #
...
class BarChart(ChartBase):
    ...
    def plot_chart(self, combined=False, stacked=False, y_log_scale=False, **kwargs):
        """ """
        #
        try:
            x_type = self._data.get_plot_data_info()["x_type"]
            ...
            if not x_type in ["", "text", "integer", "float", "date", "datetime"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.BarChart: Plot skipped, X-axis type not valid: "
                    + x_type
                )
                raise UserWarning(
                    "GraphPlotter.BarChart: Plot skipped, X-axis type not valid: "
            ...
            if not y_type in ["", "integer", "float"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.BarChart: Plot skipped, Y-axis type not valid: "
                    + y_type
                )
                raise UserWarning(
                    "GraphPlotter.BarChart: Plot skipped, Y-axis type not valid: "
            ...
            if combined:
                ...
                if y_log_scale:
                    subplot.set_yscale("log")
                    accumulatedplots = [0.1] * len(
                        x_axis
                    )  # For stacked bars. log(0) not defined.
                else:
                ...
                for plotindex, plotdict in enumerate(plotlist):
                    ...
                    if stacked:
                        ...
                        accumulatedplots = [
                            (x + y) for x, y in zip(accumulatedplots, y_array)
                        ]  # For stacked bars.
                    else:
                        rect = subplot.bar(
                            x_axis_positions + (plotindex * barwidth),
                            y_array,
                            barwidth,
                            color=colourmap(1.0 * plotindex / colourcount),
                        )
                ...
                font_properties.set_size("small")
                leg = subplot.legend(
                    [data["plot_name"] for data in plotlist],
                    loc="best",
                    fancybox=True,
                ...
                for plotindex, plotdict in enumerate(plotlist):
                    ...
                    subplot = self._figure.add_subplot(
                        subplotcount, 1, plotindex + 1, sharex=share_x_axis
                    )
                    if not share_x_axis:
                        share_x_axis = subplot
                    #
                    if y_log_scale:
                        subplot.set_yscale("log")
                    #
...
class ScatterChart(ChartBase):
    ...
    def plot_chart(
        self, combined=False, x_log_scale=False, y_log_scale=False, **kwargs
    ):
        """ """
        #
        try:
            ...
            if not x_type in ["", "text", "integer", "float", "date", "datetime"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.PieChart: Plot skipped, X-axis type not valid: "
                    + x_type
                )
                raise UserWarning(
                    "GraphPlotter.PieChart: Plot skipped, X-axis type not valid: "
            ...
            if not y_type in ["", "integer", "float"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.PieChart: Plot skipped, Y-axis type not valid: "
                    + y_type
                )
                raise UserWarning(
                    "GraphPlotter.PieChart: Plot skipped, Y-axis type not valid: "
            ...
            if not z_type in ["", "integer", "float"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.PieChart: Plot skipped, Z-axis type not valid: "
                    + z_type
                )
                raise UserWarning(
                    "GraphPlotter.PieChart: Plot skipped, Z-axis type not valid: "
            ...
            if not plotlist[0]["x_array"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.ScatterChart: Plot skipped, X values are missing."
                )
                return
            #
            if combined:
                ...
                for plotindex, plotdict in enumerate(plotlist):
                    ...
                    if (plotindex + 1) == len(plotlist):
                        ...
                        subplot.set_xticklabels(
                            plotdict["x_array"], rotation=20.0, visible=True
                        )
            #
            self._finalize_plotting()
        #
...
class BoxPlotChart(ChartBase):
    ...
    def plot_chart(
        ...
        try:
            ...
            if not y_type in ["", "integer", "float"]:
                toolbox_utils.Logging().warning(
                    "GraphPlotter.BoxPlotChart: Plot skipped, Y-axis type not valid: "
                    + y_type
                )
                raise UserWarning(
                    "GraphPlotter.BoxPlotChart: Plot skipped, Y-axis type not valid: "
...
def graphplotter_test():
    ...
    data = GraphPlotData(title="Y array only", y_type="integer", y_label="y as integer")
    data.add_plot(plot_name="First plot", y_array=[7, 88, 900, 9000])
    data.add_plot(plot_name="Second plot", y_array=[2006, 2006, 20060, 2006])
