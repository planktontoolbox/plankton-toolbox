...
class GraphPlotterTool(app_framework.ToolBase):
    ...
    def set_chart_selection(
        ...
        try:
            ...
            if comboindex >= 0:
                self._charttype_list.setCurrentIndex(comboindex)
            #
    ...
    def _create_content_labels(self):
        ...
        self._plotlabels_table.set_header(
            ["Plot name     ", "X-label     ", "Y-label     ", "Z-label     "]
        )
        ...
        try:
            try:
                ...
                if selectedchart == "Line chart":
                    self._current_chart = toolbox_utils.LineChart(
                        self._plotdata, figure=figure
                    )
                    self._current_chart.plot_chart(
                        combined=combined, stacked=stacked, y_log_scale=ylogscale
                    )
                    self._canvas.draw()
                #
                if selectedchart == "Bar chart":
                    self._current_chart = toolbox_utils.BarChart(
                        self._plotdata, figure=figure
                    )
                    self._current_chart.plot_chart(
                        combined=combined, stacked=stacked, y_log_scale=ylogscale
                ...
                if selectedchart == "Scatter chart":
                    self._current_chart = toolbox_utils.ScatterChart(
                        self._plotdata, figure=figure
                    )
                    self._current_chart.plot_chart(
                        combined=combined, y_log_scale=ylogscale
                    )
                    self._canvas.draw()
                #
                if selectedchart == "Pie chart":
                    self._current_chart = toolbox_utils.PieChart(
                        self._plotdata, figure=figure
                    )
                    self._current_chart.plot_chart()
                    self._canvas.draw()
                ...
                if selectedchart == "Boxplot chart":
                    self._current_chart = toolbox_utils.BoxPlotChart(
                        self._plotdata, figure=figure
                    )
                ...
                if selectedchart == "Map chart":
                    self._current_chart = toolbox_utils.MapChart(
                        self._plotdata, figure=figure
                    )
                    self._current_chart.plot_chart()
                    self._canvas.draw()
