...
class AnalyseDatasetsTab3(QtWidgets.QWidget):
    ...
    def content_aggregate_data(self):
        ...
        #
        self._reloaddata_button = QtWidgets.QPushButton(
            "Reload analysis data (no clean up)"
        )
        self._reloaddata_button.clicked.connect(
            self._main_activity._tab1widget._copy_datasets_for_analysis
        )
        #
        ...
        self._addmissingtaxa_trophictype_checkbox = QtWidgets.QCheckBox(
            "Include trophic types"
        )
        self._addmissingtaxa_trophictype_checkbox.setChecked(False)
        self._addmissingtaxa_stagesex_checkbox = QtWidgets.QCheckBox(
            "Include development stage/sex"
        )
            ...
            try:
                ...
                try:
                    ...
                    selected_trophic_type_list = (
                        self._trophic_type_listview.getSelectedDataList()
                    )
                    selected_trophic_type_text = "-".join(selected_trophic_type_list)
                    selected_lifestage_list = (
                        self._lifestage_listview.getSelectedDataList()
                    )
                        ...
                        for samplenode in visitnode.get_children()[:]:
                            ...
                            for variablenode in samplenode.get_children()[:]:
                                ...
                                try:
                                    ...
                                    if selected_taxon_rank == "Biota (all levels)":
                                        ...
                                        newtaxon = plankton_core.Species().get_plankton_group_from_taxon_name(
                                            variablenode.get_data("scientific_name")
                                        )
                                        ...
                                        selected_taxon_rank == "Kingdom (from dataset)"
                                    ):
                                        newtaxon = variablenode.get_data(
                                            "taxon_kingdom"
                                        )
                                        ...
                                        selected_taxon_rank == "Species (from dataset)"
                                    ):
                                        newtaxon = variablenode.get_data(
                                            "taxon_species"
                                        )
                                    ...
                                    if not newtaxon:
                                        ...
                                        newtaxon = (
                                            "not-designated"  # Use this if empty.
                                        )
                                    #
                                    ...
                                    if taxontrophic_type in selected_trophic_type_list:
                                        ...
                                        ### TEST
                                        if (
                                            newtaxon not in trophic_type_set_dict
                                        ):  ### TEST
                                            trophic_type_set_dict[
                                                newtaxon
                                            ] = set()  ### TEST
                                        trophic_type_set_dict[newtaxon].add(
                                            variablenode.get_data("trophic_type")
                                        )  ### TEST
                                    else:
                                    ...
                                    if agg_tuple in aggregatedvariables:
                                        aggregatedvariables[agg_tuple] = (
                                            value + aggregatedvariables[agg_tuple]
                                        )
                                    else:
                            ...
                            for variablekeytuple in aggregatedvariables:
                                ...
                                newvariable.add_data(
                                    "trophic_type",
                                    "-".join(
                                        sorted(trophic_type_set_dict.get(newtaxon, []))
                                    ),
                                )  ### TEST
                                ...
                                newvariable.add_data(
                                    "value", aggregatedvariables[variablekeytuple]
                                )
    ...
    def _add_missing_taxa(self):
        ...
        try:
            try:
                ...
                include_trophictypes = (
                    self._addmissingtaxa_trophictype_checkbox.isChecked()
                )
                include_stagesex = self._addmissingtaxa_stagesex_checkbox.isChecked()

                plankton_core.AnalysisPrepare().add_missing_taxa(
                    analysisdata, include_trophictypes, include_stagesex
                )
                #
                self._main_activity.update_viewed_data_and_tabs()
            except UserWarning as e:
                toolbox_utils.Logging().error(
                    "Failed to add 0 for not observed. " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self._main_activity,
                    "Warning",
