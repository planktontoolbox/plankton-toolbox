...
class AnalyseDatasetsTab8(QtWidgets.QWidget):
    ...
    def content_reports(self):
        ...
        self._report_2_button = QtWidgets.QPushButton(
            "Zooplankton: Abundance m2 and m3, length median and mean"
        )
    ...
    def _create_report_1(self):
        ...
        try:
            ...
            self.create_primer_report(
                parameters,  # Note:Support for multiple parameters.
    ...
    def _create_report_2(self):
        ...
        try:
            ...
            if not analysisdata:
                return  # Can't create a report from an empty dataset.
            # Create the report.
            self._create_report_zooplankton_abundance_length_median_and_mean(
    ...
    def create_primer_report(self, parameters, datasets, reportdata):
        """ """
        try:
            ...
            for dataset in datasets:
                for visit in dataset.get_children():
                    for sample in visit.get_children():
                        header_row_1[
                            6 + (sampleindex * numberofparameters)
                        ] = visit.get_data("station_name")
                        header_row_2[
                            6 + (sampleindex * numberofparameters)
                        ] = visit.get_data("sample_date")
                        header_row_3[
                            6 + (sampleindex * numberofparameters)
                        ] = sample.get_data("sample_min_depth_m")
                        header_row_4[
                            6 + (sampleindex * numberofparameters)
                        ] = sample.get_data("sample_max_depth_m")
                        sampleindex += 1
            #
                    ...
                    for sample in visit.get_children():
                        for variable in sample.get_children():
                            ...
                            if taxon_key not in taxon_values_dict:
                                taxon_values_dict[taxon_key] = [str()] * (
                                    numberofsamples * numberofparameters
                                )  # Add new value list.
                            #
                            for paramindex, param in enumerate(parameters):
                                ...
                                if param == (parameter + " (" + unit + ")"):
                                    taxon_values_dict[taxon_key][
                                        sampleindex * numberofparameters + paramindex
                                    ] = value
                        #
                        sampleindex += 1
            #
            ...
            tabledata.append_row(
                ["Scientific name", "Size class", "Trophic type", "Stage", "Sex", ""]
                + parameters * numberofsamples
    ...
    def _create_report_zooplankton_abundance_length_median_and_mean(
        self, dataset, reportdata
    ):
        """ """
        try:
            # Create a dataset (table, not tree).
