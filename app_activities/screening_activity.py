...
class ScreeningActivity(app_framework.ActivityBase):
    ...
    def _content_screening_tabs(self):
        ...
        tabWidget.addTab(
            self._content_codes_species_screening(), "Check species"
        )  # 'Check code lists and species')
                    ...
                    if item.checkState() == QtCore.Qt.CheckState.Checked:
                        ...
                        for visitnode in dataset.get_children():
                            ...
                            for samplenode in visitnode.get_children():
                                ...
                                row = "         - Unique parameters/units: " + str(
                                    len(parameter_set)
                                )
                                self._structureresult_list.append(row)
                                row = "         - Unique taxon names: " + str(
                                    len(taxonname_set)
                                )
                                self._structureresult_list.append(row)
                                row = (
                                    "         - Unique taxon-names/size-classes/stages/sex: "
                                    + str(len(taxonsizestagesex_set))
                                )
                                self._structureresult_list.append(row)
        #
        except Exception as e:
                    ...
                    if item.checkState() == QtCore.Qt.CheckState.Checked:
                        ...
                        for visitnode in dataset.get_children():
                            ...
                            for samplenode in visitnode.get_children():
                                ...
                                check_duplicates_list = (
                                    []
                                )  # Duplicates can occur inside one sample.
                                sample_description = (
                                    dataset_descr
                                    + ", "
                                    + visit_descr
                                ...
                                for variablenode in samplenode.get_children():
                                    ...
                                    size_class = str(
                                        variablenode.get_data("size_class")
                                    )
                                    ...
                                    if unique_items in check_duplicates_list:
                                        if sample_description:
                                            ...
                                            self._structureresult_list.append(
                                                sample_description
                                            )
                                            sample_description = ""
                                        row = (
    ...
    def _species_screening(self):
        ...
        try:
            ...
            try:
                ...
                # Perform screening.
                plankton_core.ScreeningManager().species_screening(
                    app_framework.ToolboxDatasets().get_datasets()
                )
            finally:
                # Log in result window.
    ...
    def _bvol_screening(self):
        ...
        try:
            ...
            try:
                ...
                # Perform screening.
                plankton_core.ScreeningManager().bvol_species_screening(
                    app_framework.ToolboxDatasets().get_datasets()
                )
            finally:
                # Log in result window.
    ...
    def _content_plot_parameters(self):
        ...
        self._plotparameterperdate_button = QtWidgets.QPushButton(
            "Plot parameters values per date"
        )
    ...
    def _plot_screening(self):
        ...
        try:
            ...
            graphtool.set_chart_selection(
                chart="Line chart", combined=True, stacked=False, y_log_scale=True
            )
    ...
    def _plot_screening_per_date(self):
        ...
        try:
            ...
            # View in the graph-plot tool.
            graphtool.set_chart_selection(
                chart="Scatter chart", combined=True, stacked=False, y_log_scale=True
            )
            graphtool.set_plot_data(self._graph_plot_data)
        #
        except Exception as e:
    ...
    def _add_plot_per_date(self, parameter):
        ...
        try:
            ...
            self._graph_plot_data.add_plot(
                plot_name=parameter, x_array=xarray, y_array=yarray
            )
        #
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "Warning", str(e))
