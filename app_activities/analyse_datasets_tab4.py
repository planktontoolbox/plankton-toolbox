...
class AnalyseDatasetsTab4(QtWidgets.QWidget):
    ...
    def _update_filter(self):
        ...
        try:
            ...
            self._analysisdata.set_filter_item(
                "start_date", str(self._startdate_edit.text())
            )
            self._analysisdata.set_filter_item(
                "end_date", str(self._enddate_edit.text())
            )
            self._analysisdata.set_filter_item(
                "stations", self._stations_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "visit_months", self._months_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "visits", self._visits_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "min_max_depth_m", self._minmaxdepth_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "scientific_name", self._taxon_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "trophic_type", self._trophic_type_listview.getSelectedDataList()
            )
            self._analysisdata.set_filter_item(
                "life_stage", self._lifestage_listview.getSelectedDataList()
            )
        #
        except Exception as e:
            debug_info = (
