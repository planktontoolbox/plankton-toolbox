...
class LoadDatasetsActivity(app_framework.ActivityBase):
    ...
    def __init__(self, name, parentwidget):
        ...
        plankton_core.PlanktonCounterManager().planktonCounterListChanged.connect(
            self._counter_update_dataset_list
        )
    ...
    def _content_load_dataset(self):
        ...
        try:
            ...
            tabWidget.addTab(
                self._content_plankton_counter(), "Plankton counter samples"
            )
    ...
    def _content_plankton_counter(self):
        ...
        self._cleara_metadata_button.label_clicked.connect(
            self._counter_uncheck_all_datasets
        )
        ...
        self._importcounterdataset_button = QtWidgets.QPushButton(
            "Import marked dataset(s)"
        )
        self._importcounterdataset_button.clicked.connect(
            self._counter_import_counter_datasets
        )
        self._importcounter_trophic_list_checkbox = QtWidgets.QCheckBox(
            "Update trophic types"
        )
    ...
    def _counter_update_dataset_list(self):
        ...
        try:
            ...
            for datasetname in sorted(
                plankton_core.PlanktonCounterManager().get_dataset_names()
            ):
                for samplename in sorted(
                    plankton_core.PlanktonCounterManager().get_sample_names(datasetname)
                ):
        ...
        try:
            ...
            try:
                ...
                for datasetandsample in selectedsamples:
                    ...
                    update_trophic_type = (
                        self._importcounter_trophic_list_checkbox.isChecked()
                    )
                    datasetnode = plankton_core.DataImportManager().import_dataset_file(
                ...
                toolbox_utils.Logging().error(
                    "Plankton conter file import failed on exception: " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Plankton conter file loading.\n",
                ...
                toolbox_utils.Logging().log(
                    "Importing datasets done. Number of imported datasets: "
                    + str(datasetcount)
                )
        #
        except Exception as e:
    ...
    def _content_predefined_formats(self):
        ...
        self._predefined_format_combo.setSizeAdjustPolicy(
        ...
        #                                     'PhytoWin (*.csv)']
        self._predefinedformat_list = [
            "Plankton counter sample(s) (*.xlsx)",
        ...
        self._predefined_format_combo.setCurrentIndex(
            0
        )  # 0 = 'Plankton counter sample(s) (*.xlsx)'

        # Load dataset.
        self._predefined_getdataset_button = QtWidgets.QPushButton(
            "Import datasets/datafiles..."
        )
        self._predefined_getdataset_button.clicked.connect(
            self._import_predefined_datasets
        )
        self._predefined_trophic_list_checkbox = QtWidgets.QCheckBox(
            "Update trophic types"
        )
        ...
        try:
            ...
            #             self._load_phytowin_datasets()
                ...
                QtWidgets.QMessageBox.information(
                    self, "Information", "Not implemented yet."
                )
        #
        except Exception as e:
            debug_info = (
            ...
            try:
                ...
                if filenames:
                    for filename in filenames:
                        ...
                        update_trophic_type = (
                            self._predefined_trophic_list_checkbox.isChecked()
                        )
                        datasetnode = (
                            plankton_core.DataImportManager().import_dataset_file(
                                filename,
                ...
                toolbox_utils.Logging().error(
                    "Plankton counter sample import failed on exception: " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Text file loading.\n",
    ...
    def _load_sharkweb_datasets(self):
        ...
        try:
            try:
                ...
                toolbox_utils.Logging().error(
                    "SHARKweb file import failed on exception: " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Text file loading.\n",
    ...
    def _content_textfile(self):
        ...
        self._textfile_parser_list.setSizeAdjustPolicy(
            QtWidgets.QComboBox.SizeAdjustPolicy.AdjustToContents
        )
        self._textfile_parser_list.addItems(["<select>"])
        self._textfile_parser_list.currentIndexChanged.connect(
            self._textfile_parser_selected
        )
        # - Add available dataset parsers.
        ...
        self._textfile_importcolumn_list.setSizeAdjustPolicy(
            QtWidgets.QComboBox.SizeAdjustPolicy.AdjustToContents
        )
        self._textfile_importcolumn_list.addItems(["<no parser selected>"])
        self._textfile_importcolumn_list.currentIndexChanged.connect(
            self._textfile_import_column_selected
        )
        # - Select export column:
        self._textfile_exportcolumn_list = QtWidgets.QComboBox()
        self._textfile_exportcolumn_list.setSizeAdjustPolicy(
            QtWidgets.QComboBox.SizeAdjustPolicy.AdjustToContents
        )
        ...
        self._textfile_trophic_list_checkbox = QtWidgets.QCheckBox(
            "Update trophic types"
        )
        ...
        label1 = QtWidgets.QLabel(
            "Text file character encoding (affects å, è, µ, etc.):"
        )
    ...
    def _textfile_parser_selected(self, selected_row):
        ...
        try:
            if (selected_row > 0) and (selected_row <= len(self._parser_list)):
                toolbox_utils.Logging().log(
                    "Selected parser: " + str(self._parser_list[selected_row - 1])
                )
                ...
                tablereader = toolbox_utils.TableFileReader(
                    file_path=self._parser_path,
                ...
                for row in tablereader.rows():
                    if (row[0] == "info") and (row[1] == "column_type"):
                        for index, item in enumerate(row):
                            if item == "import":
                                self._textfile_importcolumn_list.addItems(
                                    [header[index]]
                                )
                            if item == "export":
                                self._textfile_exportcolumn_list.addItems(
                                    [header[index]]
                                )
            else:
                self._textfile_importcolumn_list.clear()
    ...
    def _textfile_import_column_selected(self, selected_row):
        ...
        try:
            ...
            tablereader = toolbox_utils.TableFileReader(
                file_path=self._parser_path,
                excel_file_name=self._parser_list[
                    self._textfile_parser_list.currentIndex() - 1
                ],
            )
            header = tablereader.header()
                    ...
                    for row in tablereader.rows():
                        if (row[0] == "info") and (row[1] == "character_encoding"):
                            if row[index] and (row[index] in self._encodings_list):
                                self._textfile_encoding_list.setCurrentIndex(
                                    self._encodings_list.index(row[index])
                                )
        #
        except Exception as e:
            debug_info = (
    ...
    def _load_text_files(self):
        ...
        try:
            try:
                ...
                filenames, _filters = QtWidgets.QFileDialog.getOpenFileNames(
                    self, "Import dataset(s)", self._last_used_textfile_name, namefilter
                )
                # Check if user pressed ok or cancel.
                self._tabledataset = plankton_core.DatasetTable()
                if filenames:
                    for filename in filenames:
                        ...
                        if (
                            str(self._textfile_encoding_list.currentText())
                            == "<platform default>"
                        ):
                            textfileencoding = locale.getpreferredencoding()
                        else:
                            textfileencoding = str(
                                self._textfile_encoding_list.currentText()
                            )
                        # Set up for import file parsing.
                        importmanager = plankton_core.ImportManager(
                            str(
                                pathlib.Path(
                                    self._parser_path,
                                    str(self._textfile_parser_list.currentText()),
                                )
                            ),
                        ...
                        dataset = importmanager.import_text_file(
                            filename, textfileencoding
                        )

                        # Update trophic_type.
                        update_trophic_type = (
                            self._textfile_trophic_list_checkbox.isChecked()
                        )
                            ...
                            for sample in visit.get_children():
                                for variable in sample.get_children():
                                    ...
                                    if update_trophic_type:
                                        scientific_name = variable.get_data(
                                            "scientific_name", ""
                                        )
                                        size_class = variable.get_data("size_class", "")
                                        trophic_type = (
                                            plankton_core.Species().get_bvol_value(
                                                scientific_name,
                                                size_class,
                                                "trophic_type",
                                            )
                                        ...
                                        if trophic_type:
                                            ...
                                            )  # Use existing if not in local list.
                ...
                toolbox_utils.Logging().error(
                    "Text file import failed on exception: " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Text file loading.\n",
    ...
    def _content_xlsx(self):
        ...
        self._excel_parser_list.setSizeAdjustPolicy(
            QtWidgets.QComboBox.SizeAdjustPolicy.AdjustToContents
        )
    ...
    def _load_excel_file(self):
        ...
        try:
                    ...
                    for filename in filenames:
                        ...
                        importmanager = plankton_core.ImportManager(
                            str(
                                pathlib.Path(
                                    self._parser_path,
                                    str(self._excel_parser_list.currentText()),
                                )
                            ),
                        ...
                        # Update trophic_type.
                        update_trophic_type = (
                            self._excel_trophic_list_checkbox.isChecked()
                        )
                        print("DEBUG: excel_trophic_list")
                        for visit in dataset.get_children():
                            for sample in visit.get_children():
            ...
            #
                ...
                toolbox_utils.Logging().error(
                    "Excel file import failed on exception: " + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Excel file loading.\n",
                ...
                toolbox_utils.Logging().log_all_accumulated_rows()
                toolbox_utils.Logging().log(
                    "Importing datasets done. Number of loaded datasets: "
                    + str(datasetcount)
                )
        #
        except Exception as e:
    ...
    def _content_loaded_datasets(self):
        ...
        self._unloadmarkeddatasets_button = QtWidgets.QPushButton(
            "Remove marked dataset(s)"
        )
    ...
    def _unload_marked_datasets(self):
        ...
        try:
            ...
            for rowindex in range(rowcount):
                ...
                if self._datasets_table.getSelectionModel().isSelected(
                    self._datasets_table._tablemodel.createIndex(index, 0)
