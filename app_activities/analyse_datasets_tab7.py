...
class AnalyseDatasetsTab7(QtWidgets.QWidget):
    ...
    def content_statistics(self):
        ...
        self._parameter_list.setSizeAdjustPolicy(
        ...
        try:
                    ...
                    for group_key in grouped_size_lifestages.keys():
                        ...
                        for variablenode in grouped_size_lifestages[group_key]:
                            ...
                            if parameternadunit == selectedparameter:
                                ...
                                splitkey += ":"
                                if split_on_season:
                                    splitkey += visitseason
                                splitkey += ":"
                                if split_on_month:
                                    splitkey += visitmonth
                                splitkey += ":"
                                if split_on_station:
                                    splitkey += visitstation
                                splitkey += ":"
                                if split_on_visit:
                                    splitkey += visitvisit
                                splitkey += ":"
                                if split_on_depth:
                                    splitkey += sampleminmaxdepth
                                splitkey += ":"
                                if split_on_taxon:
                                    splitkey += variabletaxon
                                # Add data.
                                if splitkey not in data_dict:
                                    data_dict[splitkey] = []
                                data_dict[splitkey].append(
                                    variablenode.get_data("value")
                                )
            ...
            for key in sorted(data_dict.keys()):
                ...
                try:
                    ...
                    data_list = [
                        float(x) for x in data_dict[key]
                    ]  # ALT 2. list comprehensions.
                ...
                if split_on_year:
                    report_row.append(yearkey)
                if split_on_season:
                    report_row.append(seasonkey)
                if split_on_month:
                    report_row.append(monthkey)
                if split_on_station:
                    report_row.append(stationkey)
                if split_on_visit:
                    report_row.append(visitkey)
                if split_on_depth:
                    report_row.append(depthkey)
                if split_on_taxon:
                    report_row.append(taxonkey)
    ...
    def _plot_graph(self):
        ...
        try:
            ...
            graphtool.set_chart_selection(
                chart="Boxplot chart", combined=True, stacked=False, y_log_scale=False
            )
            ...
            try:
                for key in sorted(data_dict.keys()):
                    self._graph_plot_data.add_plot(
                        plot_name=key, y_array=data_dict[key]
                    )
