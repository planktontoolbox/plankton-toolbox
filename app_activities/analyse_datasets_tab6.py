...
class AnalyseDatasetsTab6(QtWidgets.QWidget):
                ...
                for visitnode in analysisdata.get_children():
                    for samplenode in visitnode.get_children():
                        for variablenode in samplenode.get_children():
                            parameterset.add(
                                variablenode.get_data("parameter")
                                + " ("
                                + variablenode.get_data("unit")
                                + ")"
                            )
    ...
    def content_generic_graphs(self):
        ...
        self._numberofvariables_list.addItems(
            [
                "One variable (Y)",
        ...
        try:
            ...
            if self._analysisdata.get_data():
                ...
                if x_selected_column == "parameter:":
                    ...
                    for (
                        item
                    ) in self._analysisdata.get_data().get_export_table_columns():
                        if item["header"] == x_selected_column:
                            self._x_axistype_list.setCurrentIndex(
                                self._type_list_values.index(item["view_format"])
                            )
                #
                if y_selected_column == "parameter:":
                    ...
                    for (
                        ...
                        if item["header"] == y_selected_column:
                            self._y_axistype_list.setCurrentIndex(
                                self._type_list_values.index(item["view_format"])
                            )
                #
                if z_selected_column == "parameter:":
                    ...
                    for (
                        ...
                        if item["header"] == z_selected_column:
                            self._z_axistype_list.setCurrentIndex(
                                self._type_list_values.index(item["view_format"])
                            )
        #
        except Exception as e:
    ...
    def _add_subplot_data(self):
        ...
        try:
            ...
            #
            plotdatainfo["x_label"] = (
                x_selected_column
                if x_selected_column != "parameter:"
                else x_selected_param
            )
            plotdatainfo["x_type"] = x_selected_type
            plotdatainfo["x_format"] = ""
            #
            plotdatainfo["y_label"] = (
                y_selected_column
                if y_selected_column != "parameter:"
                else y_selected_param
            )
            plotdatainfo["y_type"] = y_selected_type
            plotdatainfo["y_format"] = ""
            #
            plotdatainfo["z_label"] = (
                z_selected_column
                if z_selected_column != "parameter:"
                else z_selected_param
            )
            ...
            try:
                self._graph_plot_data.add_plot(
                    plot_name=plot_name,
                    x_label=x_selected_column
                    if x_selected_column != "parameter:"
                    else x_selected_param,
                    x_array=x_data,
                    y_label=y_selected_column
                    if y_selected_column != "parameter:"
                    else y_selected_param,
                    y_array=y_data,
                    z_label=z_selected_column
                    if z_selected_column != "parameter:"
                    else z_selected_param,
                    z_array=z_data,
                )
    ...
    def _get_plot_data(self):
        ...
        try:
            ...
            for visitnode in analysisdata.get_children():
                # Get data.
                if x_visit_key:
                    x_value = visitnode.get_data(x_visit_key)
                if y_visit_key:
                    y_value = visitnode.get_data(y_visit_key)
                if z_visit_key:
                    z_value = visitnode.get_data(z_visit_key)
                # Check if finished.
                ...
                for samplenode in visitnode.get_children():
                    # Get data.
                    if x_sample_key:
                        x_value = samplenode.get_data(x_sample_key)
                    if y_sample_key:
                        y_value = samplenode.get_data(y_sample_key)
                    if z_sample_key:
                        z_value = samplenode.get_data(z_sample_key)
                    # Check if finished.
                    if (y_value is not None) and (numberofvariables == 1):
                        y_data.append(y_value)
                        if y_sample_key:
                    ...
                    for variablenode in samplenode.get_children():
                        ...
                        group_key += ":" + variablenode.get_data(
                            "size_class"
                        )  # For phytoplankton
                        group_key += ":" + variablenode.get_data(
                            "stage"
                    ...
                    for group_key in grouped_lifestages.keys():
                        ...
                        for variablenode in grouped_lifestages[group_key]:
                            if x_variable_key:
                                x_value = variablenode.get_data(x_variable_key)
                            if y_variable_key:
                                y_value = variablenode.get_data(y_variable_key)
                            if z_variable_key:
                                z_value = variablenode.get_data(z_variable_key)
