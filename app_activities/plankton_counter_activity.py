...
class PlanktonCounterActivity(app_framework.ActivityBase):
    ...
    def _content_datasets_and_samples(self):
        ...
        self._counter_samples_listview.setEditTriggers(
            QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers
        )
        self._counter_samples_selection_model = (
            self._counter_samples_listview.selectionModel()
        )
        self._counter_samples_selection_model.selectionChanged.connect(
        ...
        self._exportimportsamples_button = QtWidgets.QPushButton(
            "Export/import samples (.xlsx)..."
        )
        self._exportimportsamples_button.clicked.connect(self._export_import_samples)
        self._backup_button = QtWidgets.QPushButton(
            "Export/import toolbox data (.zip)..."
        )
        ...
        try:
            ...
            if index.isValid():
                for row_index in range(index.row() + 1):
                    datasetandsamplestring = str(
                        self._counter_samples_model.item(row_index + 0, column=0).text()
                    )
                    if datasetandsamplestring.startswith("Dataset: "):
                        self._current_dataset = datasetandsamplestring.replace(
                            "Dataset: ", ""
                        ).strip()
                        self._current_sample = None
                    elif datasetandsamplestring.startswith("- Sample: "):
                        self._current_sample = datasetandsamplestring.replace(
                            "- Sample: ", ""
                        ).strip()
        #
        except Exception as e:
        ...
        try:
            ...
            if self._current_dataset is None:
                QtWidgets.QMessageBox.warning(
                    self, "Warning", "No dataset is selected. Please try again."
                )
                return
            if self._current_sample is None:
                QtWidgets.QMessageBox.warning(
                    self, "Warning", "No sample is selected. Please try again."
                )
                return
            #
            ...
            dialog = app_activities.PlanktonCounterDialog(
                self, self._current_dataset, self._current_sample
            )
            dialog.exec()
        #
        except Exception as e:
...
class NewDatasetDialog(QtWidgets.QDialog):
    ...
    def _content(self):
        ...
        formlayout.addRow(
            "", QtWidgets.QLabel("Example: SHARK_Phytoplankton_2016_NationalMonitoring")
        )
...
class NewSampleDialog(QtWidgets.QDialog):
    ...
    def _load_dataset_data(self):
        ...
        try:
            for (
                datasetname
            ) in plankton_core.PlanktonCounterManager().get_dataset_names():
    ...
    def _create_sample(self):
        ...
        try:
            ...
            #
            plankton_core.PlanktonCounterManager().create_sample(
    ...
    def _load_sample_data(self):
        ...
        try:
            ...
            for (
                ...
                for (
                    samplename
                ) in plankton_core.PlanktonCounterManager().get_sample_names(
                    datasetname
                ):
        ...
        try:
            ...
            for rowindex in range(self._samples_model.rowCount()):
                ...
                if item.checkState() == QtCore.Qt.CheckState.Checked:
                    ...
                    print(samplename)
                    plankton_core.PlanktonCounterManager().delete_sample(
                        datasetname, samplename
                    )
            #
            self.accept()  # Close dialog box.
...
class BackupExportImportDialog(QtWidgets.QDialog):
        ...
        try:
            ...
            #
                ...
                toolbox_utils.Logging().error(
                    'Failed to backup "plankton_toolbox_data". Error: ' + str(e)
                )
                QtWidgets.QMessageBox.warning(
                    self,
                    "Warning",
    ...
    def _content_from_backup(self):
        ...
        self._copyold_checkbox = QtWidgets.QCheckBox(
            "Rename the old plankton_toolbox_data before import."
        )
    ...
    def _browse_import_files(self):
        ...
        try:
            ...
            dirfilename, _filters = QtWidgets.QFileDialog.getOpenFileName(
                self, "Load backup. ", "", namefilter  # self._lastusedsharkwebfilename,
            )
            # From QString to str.
        ...
        try:
            ...
            if source_zip_dir_file_name:
                ...
                try:
                    ...
                    try:
                        toolbox_utils.Logging().error(
                            "Failed to rename plankton_toolbox_data. Check if files are locked by Excel. Error: "
                            + str(e)
                        )
                        QtWidgets.QMessageBox.warning(
                            self,
                        ...
                        toolbox_utils.Logging().error(
                            "Failed to rename plankton_toolbox_data. Check if files are locked by Excel. "
                        )  # ...Error: ' + str(e))
                        QtWidgets.QMessageBox.warning(
                            self,
                            "Warning",
                ...
                #
                    ...
                    toolbox_utils.Logging().error(
                        "Failed to import from backup.  Check if files are locked by Excel. "
                    )  # ...Error: ' + str(e))
                    QtWidgets.QMessageBox.warning(
                        self,
                        "Warning",
...
class ExportImportSamplesDialog(QtWidgets.QDialog):
    ...
    def _content_import(self):
        ...
        self._dataset_list = QtWidgets.QComboBox(self)
        self._replaceoldsamples_checkbox = QtWidgets.QCheckBox(
            "Replace samples with same name."
        )
                ...
                if item.checkState() == QtCore.Qt.CheckState.Checked:
                    ...
                    try:
                        ...
                        sample_object = plankton_core.PlanktonCounterSample(
                            dir_path, datasetname, samplename
                        )
                        #
                        sample_object.export_sample_to_excel(
                            export_target_dir, export_target_filename
                        )
                        #
                        toolbox_utils.Logging().log(
                            "Sample exported: "
                            + export_target_filename
                            + "   to directory: "
                            + export_target_dir
                        )
                    #
