...
class SharkArchive():
    ...
    def load_shark_metadata_auto(self):
        ...
        try:
            ...
            # Metadata is a key/value list with no header. Merge header and row.
            concat_table = [metadataauto_tableobject.header()] + metadataauto_tableobject.rows()
            concat_table = map('\t'.join, concat_table)
    ...
    def generate_metadata_auto(self):
        ...
        for item_index, item in enumerate(self._data_tableobject.header()):
            ...
            #
            max_index = max(year_index, date_index, latitude_index, longitude_index, parameter_index, unit_index)
        # Scan rows.
        ...
        name, datatype, version = self._split_shark_archive_filename(self._archive_filename)
    ...
    def update_archive_file(self):
        ...
        if len(entries_to_remove) > 0:
            ...
            try:
                if self._data_tableobject is not None:
                    data = '\t'.join(self._data_tableobject.header()) + '\r\n' + \
                           '\r\n'.join(map('\t'.join, self._data_tableobject.rows()))
                    zip_write.writestr('shark_data.txt', data.encode('cp1252', errors='ignore'))
                #
                if self._metadata_text is not None:
                    zip_write.writestr('shark_metadata.txt', self._metadata_text.encode('cp1252', errors='replace'))
                #
                if self._metadataauto_text is not None:
                    zip_write.writestr('shark_metadata_auto.txt', self._metadataauto_text.encode('cp1252', errors='replace'))
            finally:
                zip_write.close()
