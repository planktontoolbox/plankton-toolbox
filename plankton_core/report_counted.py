        ...
        if not aggregate_rows:
            ...
            for key in report_rows_dict.keys():
                ...
                if agg_row_key not in report_aggregated_rows_dict:
                    ...
                    try:
                        old_abundance_value = report_aggregated_rows_dict[agg_row_key].get('param_abundance', '<invalid>')
                        new_abundance_value = row_dict.get('param_abundance', '<invalid>')
                        abundance_value = float(old_abundance_value) + float(new_abundance_value)
                        report_aggregated_rows_dict[agg_row_key]['param_abundance'] = str(abundance_value)
                    except:
                        report_aggregated_rows_dict[agg_row_key]['param_abundance'] = '<invalid>'
                    #
                    try:
                        old_biovolume_value = report_aggregated_rows_dict[agg_row_key].get('param_biovolume', '<invalid>')
                        new_biovolume_value = row_dict.get('param_biovolume', '<invalid>')
                        biovolume_value = float(old_biovolume_value) + float(new_biovolume_value)
                        report_aggregated_rows_dict[agg_row_key]['param_biovolume'] = str(biovolume_value)
                    except:
                        report_aggregated_rows_dict[agg_row_key]['param_biovolume'] = '<invalid>'
            #
