    ...
    def _load_all_data(self):
        ...
        #
            ...
            toolbox_utils.Logging().error(
                "Failed when loading code lists. Exception: " + str(e)
            )
                ...
                for samplenode in visitnode.get_children():
                    ...
                    for variablenode in samplenode.get_children():
                        ...
                        if "scientific_name" in data_dict:
                            if (
                                data_dict["scientific_name"]
                                not in species.get_taxa_lookup_dict()
                            ):
            ...
            for visitnode in dataset.get_children():
                ...
                for samplenode in visitnode.get_children():
                    ...
                    for variablenode in samplenode.get_children():
                        #
                        data_dict = variablenode.get_data_dict()
                        if ("scientific_name" in data_dict) and (
                            "size_class" in data_dict
                        ):
                            taxonname = data_dict["scientific_name"]
                            sizeclass = data_dict["size_class"]

                            if (
                                species.get_bvol_value(
                                    taxonname, sizeclass, "bvol_size_class"
                                )
                                == None
                            ):
                                toolbox_utils.Logging().warning(
